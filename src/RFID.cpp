...
{
	if (!bcm2835_init())
		throw std::runtime_error(std::string("RFID.cpp:bcm2835_init() Failed!\n"));
	this->RFID_init();
}
RFID::~RFID()
{
...
void RFID::writeMFRC522(unsigned char Address, unsigned char value)
{
	char buff[2];
...
unsigned char RFID::readMFRC522(unsigned char Address)
{
    char buff[2];
...
unsigned char RFID::RFID_auth(unsigned char authMode, unsigned char BlockAddr, unsigned char *Sectorkey, unsigned char *serNum)
{
	unsigned char status;
...
unsigned char RFID::RFID_read(unsigned char blockAddr, unsigned char *recvData)
{
	unsigned char status;
	unsigned int unLen;
...
unsigned char RFID::RFID_write(unsigned char blockAddr, unsigned char *writeData)
{
	unsigned char status;
	unsigned int recvBits;
	unsigned char i;
...
unsigned char RFID::RFID_selectTag(unsigned char *serNum)
{
	unsigned char i;
	unsigned char status;
	unsigned char size;
